$(document).ready( function () {
    ...
    $('#addModal').on('show.bs.modal', function(e) {
        ...
        $.ajax({
            url: "/admin/articles/get-related/",
            method: 'POST',
            success: function (res) {
                var records = res.records
                $.each(records, function(row, field) {
                    var option = $('<option>').text(field.heading_title).val(field.id)
                    $('#add_about_item_id').append(option)
                })
            },
...
function get_all() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            var thead = $('<thead>')
            var thr = $('<tr>')
            thr.append($('<th>').text('About Title'))
            thr.append($('<th>').text('Title'))
            thr.append($('<th>').text('Text'))
            ...
            $.each(records, function(row, field) {
                ...
                tr.append($('<td>').text(field.text))
                var td_img = $('<td>')
                var img = $('<img>')
                img.attr({
                    'src' : `/uploads/articles/pictures/${field.picture}`,
                })
                td_img.append(img)
                tr.append(td_img)
...
function get_upd_id(id){
    ...
    $.ajax( {
      ...
      success: function(res) {
        ...
        $.each(records, function(row, field) {
            var option = $('<option>').text(field.heading_title).val(field.id)
            $('#upd_about_item_id').append(option)
        })

        $('#upd_about_item_id').val(record.about_item_id)
